...
* [`src/cljs/dice_and_clocks/`](src/cljs/dice_and_clocks/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/dice_and_clocks/core.cljs): contains the SPA entry point, `init`
...
The `debug?` variable in [`config.cljs`](src/cljs/dice_and_clocks/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns dice-and-clocks.example
  (:require [dice-and-clocks.config :as config])
